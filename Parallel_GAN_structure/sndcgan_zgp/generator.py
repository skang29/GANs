...
def decoder(z_, batch_size, size, gf_dim, name="decoder", sn=False, reuse=False, debug=False, tower_config=None):
    with tf.variable_scope("type__generator"):
        with tf.variable_scope(name, reuse=reuse):
            num_layers = int(np.log2(size / 4)) - 1

            if debug: print_info("Input", z_)
            nets = init_stage(z_, batch_size, gf_dim * int(2 ** num_layers), sn=sn, tower_config=tower_config)
            ...
            nets = final_stage(nets, batch_size, gf_dim, sn=sn, tower_config=tower_config)
...
def init_stage(z_, batch_size, conv_dims, name="init_stage", sn=False, tower_config=None):
...
def final_stage(nets, batch_size, conv_dims, name="final_stage", sn=False, tower_config=None):
    with tf.variable_scope(name):
        nets = up_block(nets, batch_size, conv_dims, sn=sn, tower_config=tower_config)
        nets = group_norm(nets, tower_config=tower_config)
...
def up_block(x, batch_size, conv_dims, name="upblock", sn=False, tower_config=None):
    with tf.variable_scope(name):
